...
function ProductsAdmin() {
  ...
  const save = async () => {
    ...
    const payload = { ...rest, price_inr: Number(rest.price_inr), price_usd: Number(rest.price_usd), sort_order: Number(rest.sort_order) };
    const res = id
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Ranks, tags and bundles.</p>
        </div>
        ...
        {products.map((p) => (
          <div key={p.id} className="flex items-center justify-between rounded-xl border border-border bg-card p-4">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-md" style={{ background: p.color || "#dc2626" }} />
              <div>
                <p className="font-semibold">{p.name} <span className="text-xs text-muted-foreground">/{p.slug}</span></p>
                <p className="text-xs text-muted-foreground uppercase">{p.category} · ₹{p.price_inr} · ${p.price_usd} {p.enabled ? "" : "· hidden"}</p>
              </div>
            </div>
